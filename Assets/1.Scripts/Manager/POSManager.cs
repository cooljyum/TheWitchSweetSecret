...
public class POSManager : MonoBehaviour
{
    ...
    private HashSet<PuzzleItemData> _usedFoodItems = new HashSet<PuzzleItemData>();
    ...
    //
    //    // 무작위 PuzzleItemData 선택
    //    PuzzleItemData randomPuzzleData;
    //    do
    //    {
    ...
    {
        // 현재 셀의 아이템 목록 가져오기
        List<IngredientData>[] cellItemData = GridManager.Instance.GetCellItemData();
        ...
        {
            // 레시피의 포장 구성 요소를 가져옴
            List<IngredientData> packagingComponents = recipe.packagingComponents;

            // 모든 아이템이 발견되었는지를 체크하는 변수
            bool allItemsFound = true;

            // 제거할 아이템을 저장할 리스트
            List<IngredientData> itemsToRemove = new List<IngredientData>();

            // 포장 구성 요소를 확인하여 GridManager의 아이템과 비교
            foreach (IngredientData requiredItem in packagingComponents)
            {
